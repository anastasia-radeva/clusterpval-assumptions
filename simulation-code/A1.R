...
source("~/Documents/RDirectory/clusterpval-assumptions/simulation-code/type1_est_method_functions.R")
...
sim <- sim %>% 
  generate_model(make_three_unevenly_spaced_clusters_mod_with_id, n=n, q=nfeat, 
                 len=as.list(seq(2, 6, by=2)), id=this.sim.id, 
                 sig=sig, seed=this.sim.id, vary_along = "len")
sim <- sim %>% simulate_from_model(nsim=5, index=1:10) 
